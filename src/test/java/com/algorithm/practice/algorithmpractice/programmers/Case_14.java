...
//    따라서 4를 return 합니다.
    ...
    public int solution(int n, int m, int[] section) {
        ...
        int startPoint = 0;
        int endPoint = 0;
        int lastPaintPoint = -1;
        int startPaintPoint = -1;
        boolean first = true;
        for(int i = 0; i < section.length; i++){
            if(startPoint != 0 && endPoint == 0 ) endPoint = section[i];
            if(startPoint == 0) startPoint = section[i];
            // System.out.println("startPoint: " + startPoint + " / " + "endPoint: " + endPoint+" / " + "startPaintPoint:"+ startPaintPoint + " / " + "lastPaintPoint: " + lastPaintPoint );

            if(startPoint >= startPaintPoint && startPoint <= lastPaintPoint){
                startPoint = endPoint;
                endPoint = 0;
            }

            if(startPoint != 0 && endPoint != 0){
                int between = endPoint - startPoint;
                int total = 0;
                if(first) total = between + 2;
                else total = between +1;
                if(first) first = false;

                if(total <= m){//두사이 값이 롤러보다 작거나 같으면 한번만 칠해도됨
                    answer++;
                    lastPaintPoint = startPoint  + (m-1);
                    while(lastPaintPoint > n){ //벽을 넘을 수 없음
                        lastPaintPoint--;
                    }
                    startPaintPoint = lastPaintPoint - m + 1;
                }else{//두사이 합이 롤러보다 크면 두번 칠해야함
                    answer += 2;
                    lastPaintPoint = endPoint + (m-1);
                    while(lastPaintPoint > n){ //벽을 넘을 수 없음
                        lastPaintPoint--;
                    }
                    startPaintPoint = lastPaintPoint - m + 1;
